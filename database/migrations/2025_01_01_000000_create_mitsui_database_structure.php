...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::create('campaign_premises', function (Blueprint $table) {
            ...
            $table->unique(['campaign_id', 'premise_id']);
        });
        ...
        Schema::create('campaign_years', function (Blueprint $table) {
            $table->id();
            $table->foreignId('campaign_id')->constrained('campaigns')->onDelete('cascade');
            $table->foreignId('model_year_id')->constrained('model_years')->onDelete('cascade');
            $table->timestamps();
            
            $table->unique(['campaign_id', 'model_year_id']);
        });

        // Tabla de bloqueos de horarios (blockades)
        Schema::create('blockades', function (Blueprint $table) {
            $table->id();
            $table->string('local'); // ID del local
            ...
            $table->string('reason')->nullable(); // Motivo del bloqueo
            $table->boolean('is_active')->default(true); // Estado activo
            $table->timestamps();
        });
        ...
        Schema::create('vehicles_express', function (Blueprint $table) {
            ...
            $table->string('ano'); // Año del vehículo
            $table->string('local'); // Local donde se ofrece
            $table->json('mantenimiento'); // Tipos de mantenimiento (JSON)
            $table->timestamps();
        });
        ...
        Schema::create('pop_ups', function (Blueprint $table) {
            $table->id();
            $table->string('title'); // Título del pop-up
            $table->text('description')->nullable(); // Descripción
        ...
        Schema::create('appointments', function (Blueprint $table) {
            ...
            $table->string('status')->default('scheduled'); // Estado de la cita
            $table->timestamps();
        });
