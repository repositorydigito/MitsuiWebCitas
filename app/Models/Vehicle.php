...
class Vehicle extends Model
{
    ...
    /**
     * Get the default tipo_valor_trabajo based on brand_code.
     */
    public function getTipoValorTrabajoDefaultAttribute(): string
    {
        return match ($this->brand_code) {
            'Z01' => '1085', // Toyota
            'Z02' => '1085', // Lexus (mismo tipo base)
            'Z03' => '2378', // Hino
            default => '1085',
        };
    }
    ...
    /**
     * Set the tipo_valor_trabajo based on brand_code if not provided.
     */
    public function setTipoValorTrabajoAttribute($value)
    {
        if (!empty($value)) {
            $this->attributes['tipo_valor_trabajo'] = $value;
            return;
        }

        // Si no hay valor, determinar por el código de marca
        $this->attributes['tipo_valor_trabajo'] = match ($this->attributes['brand_code'] ?? null) {
            'Z01' => '1085', // Toyota
            'Z02' => '1085', // Lexus (mismo tipo base)
            'Z03' => '2378', // Hino
            default => '1085',
    ...
    protected static function boot()
    {
        ...
        // Antes de guardar, asegurarse de que brand_name y tipo_valor_trabajo estén establecidos
        static::saving(function ($vehicle) {
            ...
            // Establecer tipo_valor_trabajo si no está definido
            if (empty($vehicle->tipo_valor_trabajo) && ! empty($vehicle->brand_code)) {
                $vehicle->tipo_valor_trabajo = match ($vehicle->brand_code) {
                    'Z01' => '1085', // Toyota
                    'Z02' => '1085', // Lexus (mismo tipo base)
                    'Z03' => '2378', // Hino
                    default => '1085',
