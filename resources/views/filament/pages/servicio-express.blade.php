        ...
        <div class="sm:w-64">
            <label for="filter-local" class="block text-sm font-medium text-gray-700 mb-2">
                ...
                @if(!empty($filtroMarca))
                    <span class="text-xs text-gray-500">({{ $filtroMarca }})</span>
                @endif
            </label>
            <select
                id="filter-local"
                ...
                class="w-full px-8 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                @if(empty($localesFiltrados)) disabled @endif
            >
                <option value="">
                    @if(empty($filtroMarca))
                        Todos los locales
                    @elseif(empty($localesFiltrados))
                        No hay locales para {{ $filtroMarca }}
                    @else
                        Todos los locales de {{ $filtroMarca }}
                    @endif
                </option>
                @foreach($localesFiltrados as $codigo => $nombre)
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($this->vehiculosPaginados as $index => $vehiculo)
                        <tr class="{{ $index % 2 === 0 ? 'bg-white' : 'bg-gray-50' }} hover:bg-gray-100">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                @if(is_array($vehiculo['year']))
                                    {{ implode(', ', $vehiculo['year']) }}
                                @else
                                    {{ $vehiculo['year'] }}
                                @endif
                            </td>
